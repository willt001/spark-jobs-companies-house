...
if __name__ == "__main__":
    ...
    companies = read_companies(spark, f's3://{BUCKET_NAME}/corporate_uk/companies.csv')
    sic_codes = read_sic_codes(spark, f's3://{BUCKET_NAME}/corporate_uk/companies_sic_codes.csv')
    filings = read_filings(spark, f's3://{BUCKET_NAME}/corporate_uk/filings.csv')
    owners = read_owners(spark, f's3://{BUCKET_NAME}/corporate_uk/officers_and_owners.csv')
    ...
    write_dataframe(company_ages, f's3://{BUCKET_NAME}/output/company_ages.csv')
    write_dataframe(account_types, f's3://{BUCKET_NAME}/output/account_types.csv')
    write_dataframe(number_of_owners, f's3://{BUCKET_NAME}/output/no_of_owners.csv')
    write_dataframe(pages_of_filings, f's3://{BUCKET_NAME}/output/pages_of_filings.csv')
    write_dataframe(current_assets, f's3://{BUCKET_NAME}/output/current_assets.csv')
    job.commit()